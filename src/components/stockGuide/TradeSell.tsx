...
const TradeSell =({handleClick}:TradeOrderProps)=>{
    ...
    <div className="order-method">
        ...
        <div className="order-margin" onClick={()=>handleClick('margin_loan')}>신용융자</div>
    </div>
    ...
    <select onChange={(e)=>selectChange(e)}>
        ...
        <div className="order-market-price" onClick={()=>handleClick('market_order')}>시장가</div>
    </div>
    <div className="order-market-sell">수량 <div className="order-market-value">0주</div></div>
    <div className="order-market-sell" style={{cursor:'pointer'}} onClick={()=>handleClick('bid_ask_spread')}>호가 <div className="order-market-value">74,000</div></div>
