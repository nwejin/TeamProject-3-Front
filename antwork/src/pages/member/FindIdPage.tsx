        ...
        try {
            event.preventDefault(); // 이벤트의 기본 동작을 취소합니다.
                ...
                if (response.success) {
                    ...
                    setFindResult(result.slice(0,result.length-1)+'*');
                } else {
    ...
    return (
        <>
            <div className="form-box">
                ...
                <>
                    <div>
                        <div className="find-box find-id" onClick={()=>findUserInfo('id')}>아이디 찾기</div>
                        <div className="find-box" onClick={()=>findUserInfo('pw')}>비밀번호 찾기</div>
                    </div>
                    <form name="register-form" method="post">
                        <input name="user_email" id="user_email" placeholder="이메일 (@포함)" className="input-box" onChange={handleInputChange}/>
                        <br/>
                        <input type="password" id="user_password" name="user_password" placeholder="비밀번호 (최소 8자리 이상)" className="input-box" onChange={handleInputChange}  />
                        <br />
                        <button className="signinBtn" onClick={(e)=>handleFindId(e,'findId')}>
                            아이디 찾기
                        </button>
                    </form>
                </>
                ...
                {findType==='pw'&&
                  <>
                  <div>
                      <div className="find-box" onClick={()=>findUserInfo('id')}>아이디 찾기</div>
                      <div className="find-box find-pw" onClick={()=>findUserInfo('pw')}>비밀번호 찾기</div>
                  </div>
                    <form name="register-form" method="post">
                    <input name="user_id" id="user_id" placeholder="아이디 (최소 4자리 이상)" className="input-box" onChange={handleInputChange}/>
                    <br/>
                    <input name="user_email" id="user_email" placeholder="이메일 (@포함)" className="input-box" onChange={handleInputChange}/>
                    <br/>
                    <button className="signinBtn" onClick={(e)=>handleFindPw(e,'findPw')}>
                        비밀번호 찾기
                    </button>
                </form>
                </>
                ...
                {findType==='pwReset'&&
                  ...
                  <form name="register-form" method="post">
                    <input type="password" id="user_password" name="user_password" placeholder="비밀번호 (최소 8자리 이상)" className="input-box" onChange={handleInputChange} />
                    <br />
                    <input type="password" id="user_pwCheck" name="user_pwCheck" placeholder="비밀번호 확인" className="input-box" onChange={handleInputChange} onKeyUp={passwordReCheck} ref={pwRef}/>
